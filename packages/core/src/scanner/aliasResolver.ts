...
export class AliasResolver {
  ...
  constructor(projectRoot: string) {
    ...
    this.baseUrl = null;
    this.extensions = ['.jsx', '.js', '.tsx', '.ts', '.mjs', '.vue', '.json'];
    this.init();
  }
    ...
    for (const name of ['vite.config.js', 'vite.config.mjs', 'vite.config.ts']) {
      ...
      if (fs.existsSync(p)) {
        try {
          ...
          if (aliasBlockMatch && aliasBlockMatch[1]) {
            const aliasBlock = aliasBlockMatch[1];
            const regex =
              /['"]?([a-zA-Z0-9_\-@]+)['"]?\s*:\s*(?:path\.resolve\([^,]+,\s*['"]([^'"]+)['"]\)|['"]([^'"]+)['"])/g;
            ...
            while ((match = regex.exec(aliasBlock)) !== null) {
              ...
              if (
                aliasKey &&
                relativeTarget &&
                !['react', 'react-dom', 'react-slick', 'vue'].includes(aliasKey)
              ) {
