...
Для React 17–19 и Vue 3 доступны статические DevTools, наблюдение в браузере и адаптер для dev-сборки. Подключение runtime и измерение библиотек описаны в [руководстве Stage 2](docs/stage-2-devtools.md). Сборка целевого проекта запускается только кнопкой **Measure Vite build**.

Для типового Vite-проекта кнопка **Add DevTools command** в панели **Runtime & Libraries** добавляет команду `dev:grimoire` и подключает адаптер к точке входа. После этого запустите `npm run dev:grimoire` в целевом проекте. Кнопка **Remove DevTools setup** удаляет эту интеграцию.
...
4. Запустит локальный сервер на `http://localhost:4173` (или ближайшем свободном порту).
